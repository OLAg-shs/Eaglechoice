-- ============================================================
-- EAGLE CHOICE - Master Database Reset & Full Schema
-- ============================================================
-- WARNING: This script drops all existing tables and data
-- in the public schema and recreates everything from scratch,
-- including the new Agent Products & Verification features.
-- ============================================================
...
CREATE TABLE public.profiles (
  ...
  is_verified BOOLEAN DEFAULT false, -- NEW: badge for verified agents
...
CREATE TABLE public.products (
  ...
  client_id UUID REFERENCES public.profiles(id) ON DELETE SET NULL, -- NEW: link product to agent
  name TEXT NOT NULL,
  description TEXT,
...
-- Cleanup existing trigger on auth.users if it exists
...
CREATE POLICY "products_select_available" ON public.products
  ...
  USING (is_available = true OR public.is_admin() OR client_id = auth.uid());
...
-- NEW CLIENT POLICIES FOR PRODUCTS
...
CREATE POLICY "Clients can update own products" ON public.products
  FOR UPDATE TO authenticated
  USING (auth.uid() = client_id AND public.is_client())
  WITH CHECK (auth.uid() = client_id AND public.is_client());

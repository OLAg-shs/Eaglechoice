            ...
            <TableRow key={service.id} className="border-gray-100 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
              ...
              <TableCell className="text-right space-x-1">
                <Button 
                  size="icon" 
                  variant="ghost" 
                  className={service.is_available ? "text-green-600 hover:text-green-700 hover:bg-green-50 dark:hover:bg-green-500/10" : "text-gray-400 hover:text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-500/10"}
                  onClick={() => handleToggleStatus(service.id, service.is_available)}
                  disabled={loadingId === service.id}
                  title={service.is_available ? "Currently Published (Click to Hide)" : "Currently Hidden (Click to Publish)"}
                >
                  {loadingId === service.id ? <Loader2 className="h-4 w-4 animate-spin" /> : (service.is_available ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />)}
                </Button>
